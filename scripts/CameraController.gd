...
func _process(delta: float) -> void:
	...
	if character_entity.lock_target:
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseMotion:
		...
		if character_entity.lock_target and event.relative.length() >= swap_threshold * screen_width:
...
func _set_marker_position():
	marker.global_position = character_entity.lock_target.global_position


func _look_at_target(delta: float) -> void:
	var desired_dir: Vector3 = (character_entity.lock_target.global_position - global_position).normalized()
	...
	var cam_dir: Vector3 = (character_entity.lock_target.global_position - camera.global_position).normalized()
...
func set_lock_target(_target: Node3D):
	character_entity.set_lock_target(_target)
	if _target == null:
...
func _switch_lock_target(mouse_delta: Vector2) -> void:
	...
	var current_screen_pos: Vector2 = camera.unproject_position(character_entity.lock_target.global_position)
	var best: Node3D = _get_best_target(current_screen_pos, mouse_delta, character_entity.lock_target)
	if best:
